...
const BarangItem = ({ product, index, onDelete, onEdit }) => {
  ...
  const getHargaBySatuan = () => {
    const satuan = product?.data?.satuan_utama;

    if (satuan === "pcs") {
      return product?.data?.harga_barang || 0;
    }
    if (satuan === "dus") {
      return product?.data?.harga_pcs || 0;
    }
    if (satuan === "kg") {
      return product?.data?.harga_per_kg || 0;
    }
    ...
    <>
      <tr className="border-b border-gray-100 hover:bg-gray-50 transition">
        ...
        <td className="text-center py-2 px-4 text-gray-700">
          Rp {getHargaBySatuan().toLocaleString()}
        </td>
        <td className="text-center py-2 px-4 text-gray-700">
          {product.data.stok_barang}
        </td>
        <td className="text-center py-2 px-4">{product?.data.satuan_utama}</td>
